...
const BudgetModal = ({ isOpen, onClose, onSubmit, category }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-40 p-4">
      <Card className="w-full max-w-md">
        <h3 className="text-xl font-bold text-white mb-4">
          {category ? 'Edit' : 'Add'} Budget Category
        </h3>
...
const Budget = ({ userData, onUserDataChange, onDeleteCategory }) => {
  ...
  const handleSaveCategory = (category) => {
    ...
    if (category.id) {
      updatedCategories = userData.budgetCategories.map(c =>
        c.id === category.id ? { ...c, name: category.name, allocated: category.allocated } : c
      );
    } else {
      const newCategory = {
        ...category,
                ...
                <div className="flex justify-between items-center mb-1">
                  <div className="flex-grow">
                    ...
                    <p className="text-sm text-gray-400">
                      ${cat.spent.toFixed(2)} / ${cat.allocated.toFixed(2)}
                    </p>
                  </div>
                  <div>
                    <button
